function [QQ, RR] = bcgs_iro_ls_mp(XX, s, ~, verbose)
% [QQ, RR] = BCGS_IRO_LS_MP(XX, s, ~, verbose) performs Block Classical
...
% Selectively uses quad precision via Advanpix
%
...
for k = 2:p
    ...
    if k == 2
        R_tmp = mp(Q_tmp',34) * mp([Q_tmp Xk],34); % quad precision
    else
        tmp = mp([QQ(:, 1:sk-2*s) Q_tmp]',34) * mp([Q_tmp Xk],34); % quad precision
        W = tmp(1:sk-2*s, s1);
        Z = tmp(1:sk-2*s, s2);
    ...
    R_diag = chol_free_mp(R_tmp(:, s1)); % quad precision
    ...
    if k == 2
        ...
        QQ(:,kk-s) = double(mp(Q_tmp,34) / mp(R_diag,34)); % quad precision
    else
        % Assign finished entries of RR
        ...
        % Finish normalizing QQ(:,k-1)
        QQ(:,kk-s) = double(mp((Q_tmp - QQ(:, 1:sk-2*s) * W),34) / mp(R_diag,34)); % quad precision
    end
...
tmp = mp([QQ(:,1:n-s) Q_tmp]',34) * mp(Q_tmp,34); % quad precision
W = tmp(1:n-s,:);
R_tmp = tmp(end-s+1:end,:) - W' * W; % quad precision
R_diag = chol_free_mp(R_tmp); % quad precision
RR(kk, kk) = double(R_diag);
RR(1:n-s, kk) = RR(1:n-s, kk) + double(W);
QQ(:,kk) = double(mp((Q_tmp - QQ(:,1:n-s) * W),34) / mp(R_diag,34)); % quad precision
