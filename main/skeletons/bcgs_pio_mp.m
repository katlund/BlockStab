function [QQ, RR] = bcgs_pio_mp(XX, s, musc, verbose, param)
% [QQ, RR] = BCGS_PIO(XX, s, musc, verbose, param) performs Block Classical
...
for k = 2:p
    ...
    tmp = tmp' * tmp;
    R_diag = chol_free_mp( tmp(1:s,1:s) - tmp(end-s+1:end, end-s+1:end), param );
