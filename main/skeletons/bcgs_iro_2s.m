...
for k = 2:p
    ...
    if k == 2
        ...
        [QQ(:,kk-s), RR(kk-s, kk-s)] = IntraOrtho(Q_tmp, musc); % 1
        S = RR(kk-s, kk-s)' \ ( Q_tmp' * Xk ); % 2
    ...
    elseif k >= 3
        % Sync point split to help with stability
        tmp = [QQ(:, 1:sk-2*s) Q_tmp]' * [Q_tmp Xk]; % 1
        W = tmp(1:sk-2*s, s1);
        Z = tmp(1:sk-2*s, s2);
        ...
        [QQ(:,kk-s), RR(kk-s, kk-s)] = IntraOrtho((Q_tmp - QQ(:, 1:sk-2*s) * W), musc); % 2

        % Update S (note that it increases by an s x s block)
        S = [Z; RR(kk-s, kk-s)' \ R_tmp(:, s2)];
