function [QQ, RR, TT] = bcgs_iro_t(XX, s, IOstr, verbose)
% [QQ, RR, TT] = BCGS_IRO_T(XX, s, IOstr, verbose) performs the T-variant of
% BCGS_IRO on the m x n matrix XX with p = n/s block partitions each of
% size s and with intra-orthogonalization procedure determined by IOstr.
%
...
for k = 1:p-1
    ...
    % First BCGS step
    RR1 = TT(1:sk,1:sk)' * InnerProd(QQ(:,1:sk), W, IOstr);
    W = W - QQ(:,1:sk) * TT(1:sk,1:sk) * RR1;
    [W, R1, T1] = IntraOrtho(W, IOstr);
