function RunTestGlued(XXdim, singval, skel, musc)
% RUNTESTGLUED(XXdim, singval, skel, s_bgs) is a wrapper function
...
for i = 1:nmat
    ...
    sglued = p;
    pglued = s;
    matstr = sprintf('%s_cond%d_m%d_p%d_s%d.mat', fstr, singval(i), m, pglued, sglued);
    cd matrices
    if exist(matstr, 'file')
        load(matstr, 'XX')
    else
        XX = CreateGluedMatrix(.5*singval(i), singval(i), m, pglued, sglued);
